...
module ApplicationHelper
  ...
  def avatar_thumb(size, person, avatar_html_options={})
    ...
    image_url = (person.image.present? ? person.image.url(size) : missing_avatar(size)) if person && image_url.nil?
  ...
  def large_avatar_thumb(person, options={})
    image_url = person.site_logo if person && !person.site_logo.nil?
    image_url = (person.image.present? ? person.image.url(:medium) : missing_avatar(:medium))   if person && image_url.nil?
